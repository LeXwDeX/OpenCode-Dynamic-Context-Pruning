...
- **`lib/hooks.ts`** — Hook factories: the transform handler (fail-open wrapper around the engine), the `chat.params` handler (learns `model.limit.context` per session), the compacting handler (arms the one-shot DTC skip, then applies the semantic checkpoint prompt with the previous checkpoint fetched via `client.session.messages` and embedded as `<previous-checkpoint>` — the host hides prior checkpoint pairs from the summarizer input and DCP's prompt replacement bypasses the host's `buildPrompt({previousSummary})`, so this fetch is what keeps checkpoints rolling), the slim event handler (`session.deleted` → drop state), the `/dcp fold|status` command handler, and the `config` hook (registers `/dcp` when enabled; always raises host `compaction.tail_turns`/`preserve_recent_tokens` to 4/32000 defaults without overriding explicit user config — this shapes the host's own overflow-compaction fallback).
...
- **`lib/prompts/compaction.ts`** — Bilingual (`language` config: `zh` default / `en`) four-section checkpoint prompt for the HOST's own compaction (manual `/compact` or overflow fallback): 历史概要 → 已完成任务的概括 → 进行中任务详情 → 未解决问题. Encodes the tiered rules (distant heavy / recent light), the `<previous-checkpoint>` rolling-merge contract, and the instruction to never restate the host-retained tail or system-level content.
...
- **`lib/config.ts`** — Config resolution: global `~/.config/opencode/dcp.jsonc` → `$OPENCODE_CONFIG_DIR` → project `.opencode/dcp.jsonc` (`.jsonc` or `.json`, layered merge). The `dtc` block mirrors the engine's `DtcConfig` plus `enabled`. Legacy `summarize.*` and `autoPrune.*` keys are deprecated-but-recognized; `autoPrune.driftThreshold` migrates to `dtc.driftThreshold`. Adding a config key means updating `VALID_CONFIG_KEYS`, defaults, merge functions, and `validateConfigTypes` here **and** the root `dcp.schema.json` (its `$id` is referenced by generated user configs).
...
- `tests/dtc-engine.test.ts` pins the engine invariants: T-zone never touched at any level, structural invariant (message/part counts, IDs, types, order byte-identical after folding), fail-open paths (short session / unknown window / compaction skip), digest shape + host-native `time.compacted` marker, escalation stops at target, manual boundary marks deepen folding below the watermark, error-state tool parts never fold, malformed-message tolerance, and a 1000-message latency bound.
...
- `tests/config-migration.test.ts` covers legacy `compress.*` / `summarize.*` / `autoPrune.*` keys: recognized (migration hint) but ignored — never re-implement them; `autoPrune.driftThreshold` migrates to `dtc.driftThreshold`.
...
- **Never touch the session state machine.** DCP must not call `session.summarize` or any session-mutating endpoint, must not create messages/parts, and must not depend on busy/idle gating — compression lives entirely in the request-scoped `experimental.chat.messages.transform` hook. The host's own compaction (manual `/compact`, overflow fallback) is the only checkpoint writer; DCP only supplies its prompt and tail-protection config defaults.
- **Never break structure.** The transform may only rewrite string payloads (`text`, `state.output`, reasoning) and set the host-native `state.time.compacted` marker / clear `state.input`. Messages and parts are never added, removed, reordered, or re-IDed — tool-call↔tool-result pairing must be structurally unbreakable. No custom placeholders, anchors, block graphs, or persisted markers; no plugin checkpoint persistence or per-message IDs; no normal chat/system message injection.
...
- **Fail open everywhere.** Unknown context window → no folding; engine errors → the request proceeds unfolded; compaction-prompt failures → native compaction continues with its default prompt; the host's compaction input is skipped via the one-shot flag so summarizer fidelity is never reduced.
